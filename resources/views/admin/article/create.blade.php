<x-app-layout head="Tambah Article" title="Admin - Tambah Article">
            ...
            <form action="{{ route('template.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div x-data="{tab : 'content'}" class="space-y-4 sm:space-y-6">
                    ...
                    <div class=" w-full grid grid-cols-2 gap-4">
                        <button @click="tab ='content'" type="button" :class="tab === 'content' ? 'border-byolink-1 text-byolink-1' : 'text-neutral-500 border-neutral-400 hover:text-black hover:border-black'" class=" w-full duration-300 border-b-2 pb-4 font-bold">Konten</button>
                        <button @click="tab ='image'" type="button" :class="tab === 'image' ? 'border-byolink-1 text-byolink-1' : 'text-neutral-500 border-neutral-400 hover:text-black hover:border-black'" class=" w-full duration-300 border-b-2 pb-4 font-bold">Image</button>
                    </div>
                    ...
                    <div x-show="tab === 'image'" class=" space-y-4 sm:space-y-6">
                        ...
                        <div x-data="imageGallery" class="flex flex-col gap-2">
                            <label class=" text-sm sm:text-base font-medium" for="image_gallery">Galeri (Max 10)</label>
                        ...
                        <script>
                            function imageGallery() {
                                return {
                                    ...
                                    previewImages(event) {
                                        const files = Array.from(event.target.files).slice(0, 10 - this.images.length);
                                        files.forEach(file => {
                                            const url = URL.createObjectURL(file);
                                            this.images.push(url);
                                        });
