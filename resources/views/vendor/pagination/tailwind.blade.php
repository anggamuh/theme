    ...
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        @if ($paginator->hasPages())
            ...
            <nav class=" w-full flex items-center justify-center space-x-1 mt-4" role="navigation" aria-label="Pagination Navigation">
                ...
                @if ($currentPage > 1)
                    <a href="{{ str_replace('?page=', '/',$paginator->url(1)) . (request('search') ? '?search=' . urlencode(request('search')) : '') }}"
                ...
                @if (!$paginator->onFirstPage())
                    <a href="{{ str_replace('?page=', '/',$paginator->previousPageUrl()) . (request('search') ? '?search=' . urlencode(request('search')) : '') }}"
                        class="  w-7 sm:w-9 aspect-square p-2 sm:p-3 rounded-md bg-neutral-100 text-neutral-600 hover:bg-byolink-1 hover:text-white duration-300"
                ...
                @for ($i = $start; $i <= $end; $i++)
                    ...
                    @else
                        <a href="{{ str_replace('?page=', '/', $paginator->url($i)) . (request('search') ? '?search=' . urlencode(request('search')) : '') }}"
                ...
                @if ($paginator->hasMorePages())
                    <a href="{{ str_replace('?page=', '/',$paginator->nextPageUrl()) . (request('search') ? '?search=' . urlencode(request('search')) : '') }}"
                        class="  w-7 sm:w-9 aspect-square p-2 sm:p-3 rounded-md bg-neutral-100 text-neutral-600 hover:bg-byolink-1 hover:text-white duration-300"
                ...
                @if ($currentPage < $lastPage)
                    <a href="{{ str_replace('?page=', '/',$paginator->url($lastPage)) . (request('search') ? '?search=' . urlencode(request('search')) : '') }}"
                        class=" w-7 sm:w-9 aspect-square p-2 sm:p-3 rounded-md bg-neutral-100 text-neutral-600 hover:bg-byolink-1 hover:text-white duration-300"
