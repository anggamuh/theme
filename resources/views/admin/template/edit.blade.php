        ...
        <div class="w-full p-4 sm:p-6 rounded-md shadow-md shadow-black/20 relative overflow-hidden">
            ...
            <form action="{{ route('template.update', ['template' => $template->id]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class=" space-y-4 sm:space-y-6 relative">
                    ...
                    <div class=" w-full">
                        <div class=" w-full flex items-center justify-center">
                            <div id="desc" style="background-color: {{$template->desc_main_color ?? ''}}; color: {{$template->desc_text_color ?? ''}}" class=" max-w-[400px] w-full rounded-md shadow-md p-4 space-y-2 overflow-hidden relative">
                                @include('components.admin.template.article')
                                <div class=" w-full flex flex-wrap gap-2">
                                    <a href="{{route('category', ['category' => 'makam-svargaloka'])}}">
                                        <button id="tagdesc" style="background-color: {{$template->desc_second_color ?? ''}}" class=" px-2 py-1 text-xs text-white rounded-md">Makam Svargaloka</button>
                                    </a>
                                </div>
                                <p class="text-lg font-bold">Biaya Pemakaman Premium Dekat Di Jogja Svargaloka Memorial Garden</p>
                                <div class=" flex gap-4 items-center text-sm">
                                    <div class=" flex gap-1.5 items-center">
                                        <div id="descicon" style="color: {{$template->desc_second_color ?? ''}}" class=" w-4 aspect-square">
                                ...
                                <style>
                                    .article a {
                                        font-weight: 700;
                                        color: {{$template->desc_second_color ?? ''}};
                                    }
                                </style>
                            </div>
                        </div>
                        ...
                        <div class="w-full flex items-center justify-center">
                            <div class=" w-full max-w-[400px] relative">
                                ...
                                <div class=" w-full grid grid-cols-2 gap-2 text-sm">
                                    <button id="phone" type="button" style="background-color: {{$template->contact_main_color ?? ''}}" class=" w-full flex items-center justify-center gap-0.5 py-2 text-white rounded-full">
                                    ...
                                    <button id="wa" type="button" style="background-color: {{$template->contact_second_color ?? ''}}" class=" w-full flex items-center justify-center gap-0.5 py-2 text-white rounded-full">
                                        <div class=" w-4 aspect-square">
